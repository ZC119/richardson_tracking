...
function [precision, fps] = run_tracker_1(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	padding = 1.5;  %extra area surrounding the target % 1.5倍的bounding box
	lambda = 1e-4;  %regularization % 最小二乘正则化系数
	...
	case 'choose',
		...
		if ~isempty(video),
			[precision, fps] = run_tracker_1(video, kernel_type, ...   % 跟踪函数 返回精度与fps
	...
	case 'all',
		...
		if ~exist('matlabpool', 'file'),
			...
			for k = 1:numel(videos),
				[all_precisions(k), all_fps(k)] = run_tracker_1(videos{k}, ...
	...
	otherwise
		...
		%call tracker function with all the relevant parameters
		[positions, time] = tracker_1(video_path, img_files, pos, target_sz, ...
			padding, kernel, lambda, output_sigma_factor, interp_factor, ...
